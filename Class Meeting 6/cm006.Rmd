...
```{r}
select(gapminder, continent, everything()) #everything() selects all of the other columns
```
...
arrange(select(gapminder, year, lifeExp, country), year, lifeExp) #selecting the columns first, and then arranging based on desired columns 
```
...
```{r}
gapminder %>%
  transmute(gdpBill = gdpPercap*pop/100000000)
```
